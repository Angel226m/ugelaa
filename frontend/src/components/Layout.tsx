      ...
      <div className={`transition-all duration-300 ${!isMobile && !sidebarVisible ? 'lg:ml-0' : !isMobile ? (isCollapsed ? 'lg:ml-20' : 'lg:ml-72') : ''}`}>
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-2xl border-b border-slate-200/60 shadow-sm">
          <div className="flex items-center justify-between px-4 lg:px-8 h-16">
            <div className="flex items-center gap-4">
              ...
              <div className="hidden sm:block">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-base font-bold text-slate-800">Planillas SU</h2>
                    <p className="text-[10px] text-slate-500 -mt-0.5">{new Date().toLocaleDateString('es-PE', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button className="relative p-2.5 rounded-xl hover:bg-cyan-50 text-slate-500 hover:text-cyan-600 transition-all">
              ...
              <div className="relative">
                <button 
                  ...
                  className="flex items-center gap-2 pl-2 hover:bg-slate-50 rounded-xl py-1.5 pr-2 transition-all"
                >
                  <div className="w-9 h-9 bg-gradient-to-br from-cyan-400 to-blue-500 rounded-lg flex items-center justify-center text-white font-bold text-sm shadow-md">
                    A
                  </div>
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50">
                    <div className="px-4 py-2 border-b border-slate-100 mb-1">
                      ...
                      <p className="text-xs text-slate-500">admin@planillas.su</p>
                    </div>
                    <button className="w-full flex items-center gap-3 px-4 py-2.5 text-slate-600 hover:bg-slate-50 transition-all">
                      <User className="w-4 h-4" />
                      <span className="text-sm">Mi Perfil</span>
                    </button>
                    <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-2.5 text-red-500 hover:bg-red-50 transition-all">
