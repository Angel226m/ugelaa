...
import { Search, Plus, Pencil, Trash2, X, User, Briefcase, Hash, Building, Tag, CheckCircle, XCircle, UserCheck, Users, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react'
...
export default function Personal() {
  ...
  const loadPersonal = () => {
    setLoading(true)
    personalApi.list(debouncedSearch, page, 20)
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center bg-slate-100 rounded-xl p-1">
              ...
              >
                <Users className="w-3 h-3" /> Todos <span className="bg-slate-200 px-1.5 py-0.5 rounded text-[10px]">{total}</span>
              </button>
              <button
                onClick={() => { setFilterStatus('active'); setPage(1) }}
                className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 ${
                  filterStatus === 'active' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                <CheckCircle className="w-3 h-3" /> Activos <span className={`px-1.5 py-0.5 rounded text-[10px] ${filterStatus === 'active' ? 'bg-white/20' : 'bg-emerald-100 text-emerald-700'}`}>{activeCount}</span>
              </button>
              <button
                onClick={() => { setFilterStatus('inactive'); setPage(1) }}
                className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 ${
                  filterStatus === 'inactive' ? 'bg-slate-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                <XCircle className="w-3 h-3" /> Inactivos <span className={`px-1.5 py-0.5 rounded text-[10px] ${filterStatus === 'inactive' ? 'bg-white/20' : 'bg-slate-200 text-slate-600'}`}>{total - activeCount}</span>
              </button>
            </div>
          </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50/50">
                <span className="text-xs text-slate-500">
                  Mostrando <span className="font-semibold text-slate-700">{(page - 1) * 20 + 1}</span> - <span className="font-semibold text-slate-700">{Math.min(page * 20, total)}</span> de <span className="font-semibold text-cyan-600">{total}</span> empleados
                </span>
